...
@app.route('/favourites/days', methods=['POST', 'GET'])
def FavouritesByDay():
    count = query.GetAllFavouritesCounts()
    days = query.GetAllFavouritesGroupByDay()
    
    if request.method == 'POST':
        
        id = request.form.get('dayselect')
        #day_id = request.form.get('sdselect') 
               
        
        return redirect(url_for('FavouritesAllByDayId', dayid=id))
    
    
    return render_template('faves.html', counts=count, days=days)

@app.route('/favourites/translation/all', methods=['POST', 'GET'])
def FavouritesForTranslationAll():
    
    vehicles = query.TranslationGetAll()
...
@app.route('/all/today', methods=['POST','GET'])
def AllVehicles():
    
    userlist = query.GetAllUsers()
    
    cursor = query.GetAllAuctionvehiclesToday()
    ...
    return render_template('vehicle.html', vehicles=cursor, userlist=userlist)



@app.route('/favourites/all/<dayid>', methods=['POST','GET'])
def FavouritesAllByDayId(dayid):
    
    cursor = query.GetAllFavouritesToday(None, dayid)
    
    return render_template('vehicle.html', vehicles=cursor)

@app.route('/favourites/lastXdays/<days>', methods=['POST','GET'])
def FavouritesLastXdays(days):
    
    cursor = query.GetAllFavouritesLastXDays(None, days)
